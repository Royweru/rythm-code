      ...
      <div className="flex min-h-16 w-full items-center justify-between gap-4 px-5 md:min-h-18 md:px-8 lg:w-auto">
        ...
        >
          ...
          >
            <div
              ref={menuRef}
              className="flex w-full flex-col border border-t-0 border-border-primary rounded-lg
               bg-blue-dark3  p-5 md:p-8 lg:w-auto lg:flex-row lg:border-none lg:bg-none lg:p-0"
            >
              {navLinks.map((navLink, index) => (
                <div key={index}>
                  {navLink.subMenuLinks && navLink.subMenuLinks.length > 0 ? (
                    ...
                    <a
                      href={navLink.url}
                      className="relative text-white block py-3 text-center text-md lg:px-4 lg:py-2 lg:text-left lg:text-base font-semibold"
                    >
                      {navLink.title}
                    </a>
                  )}
...
const SubMenu = ({
  ...
  return (
    ...
    >
      ...
      >
        <span className=" text-white font-semibold">{navLink.title}</span>
        <motion.span
          variants={{
            rotate: { rotate: 180 },
            initial: { rotate: 0 },
          ...
          transition={{ duration: 0.3 }}
        >
          <RxChevronDown className=" text-white size-4" />
        </motion.span>
      </button>
      <AnimatePresence>
        {isDropdownOpen && (
